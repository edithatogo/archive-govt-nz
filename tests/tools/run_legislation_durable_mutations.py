"""Run bounded durable-package integrity mutants, preserving all attempt evidence."""
...
def main() -> int:
    ...
    source = Path("tools/legislation_durable_state.py").read_text()
    definitions = json.loads(
        Path("tests/tools/legislation_durable_mutants.json").read_text()
    )
    for name in (
        "legislation_parent_state",
    ...
    command = [
        ...
        "-m",
        "pytest",
        "tests/tools/test_legislation_durable_state.py",
        "-q",
        "--no-cov",
        "--maxfail=1",
    ]
    ...
    for index, mutant in enumerate(definitions):
        ...
        environment = dict(os.environ, DURABLE_UNDER_TEST=str(path.resolve()))
        ...
        outcomes.append(
            {
                ...
                and b"FAILED tests/tools/test_legislation_durable_state.py" in log

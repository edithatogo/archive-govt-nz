"""Run bounded source mutations against the fail-closed policy tests."""
...
SOURCE_RELATIVE = Path("src/archive_govt_nz/resource_policy.py")
TEST_ARGUMENTS = ("tests/policy/test_resource_policy.py", "-q")
MUTATIONS = {
    "scheme_acceptance": ('parsed.scheme != "https"', 'parsed.scheme == "https"'),
    "redirect_limit": (
        "len(candidate.redirect_urls) > config.max_redirects",
        "len(candidate.redirect_urls) < config.max_redirects",
    ),
    "restricted_rights": (
    ...
    "rate_limit": (
        "candidate.status_code == _RATE_LIMITED",
        "candidate.status_code != _RATE_LIMITED",
    ),
    "type_conflict": (
        "candidate.declared_media_type != candidate.magic_type",
        "candidate.declared_media_type == candidate.magic_type",
    ),
    "member_limit": (
        "candidate.archive_member_count > active.max_archive_members",
        "candidate.archive_member_count < active.max_archive_members",
    ),
    "expansion_limit": (
        "candidate.expansion_ratio > active.max_expansion_ratio",
        "candidate.expansion_ratio < active.max_expansion_ratio",
    ),
    "eligible_reason": ('"preflight_passed"', '"mutated_preflight"'),
}
...
def main() -> int:
    ...
    output = REPOSITORY_ROOT / "build" / "mutation-resource-policy.json"

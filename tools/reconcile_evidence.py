...
def main() -> int:
    ...
    resolution = load(
        root
        / "conductor/tracks/treasury_archive_mvp_20260731/evidence/phase-10-source-resolution-manifest.json"
    )
    tombstones = load(root / "evidence/phase-10-tombstone-reprobe.json")
    release = load(
        root
        / "conductor/tracks/treasury_archive_mvp_20260731/evidence/phase-9-release-reconciliation.json"
    )
    ...
    result = {
        ...
        "limitations": [
            "Payload availability and live WARC coverage remain externally gated; reconciliation does not assert complete capture."
        ],
    }

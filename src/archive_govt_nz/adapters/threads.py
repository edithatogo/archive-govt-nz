"""Threads public post capture adapter."""
...
class ThreadsCaptureAdapter(AsyncBaseCaptureAdapter):
    """Adapter for archiving public Threads accounts and posts."""
    ...
    def adapter_name(self) -> str:
        ...
        return "archive-govt-nz/capture/threads:0.1.0"

    async def capture(self, identity: SourceIdentity) -> AdapterCaptureResult:
        """Fetch public profile/posts and store payload in CAS."""
        if identity.source_type != SourceType.THREADS:
        ...
        try:
            ...
            record = self.store_payload(
                content, media_type="text/html", uri=profile_url
            )

"""YouTube channel and video metadata capture adapter."""
...
class YouTubeCaptureAdapter(AsyncBaseCaptureAdapter):
    """Adapter for archiving YouTube channel RSS feeds and metadata."""
    ...
    def adapter_name(self) -> str:
        ...
        return "archive-govt-nz/capture/youtube:0.1.0"

    async def capture(self, identity: SourceIdentity) -> AdapterCaptureResult:
        """Fetch YouTube channel XML feed and store in CAS."""
        if identity.source_type != SourceType.YOUTUBE:
        ...
        try:
            ...
            record = self.store_payload(
                content, media_type="application/xml", uri=feed_url
            )

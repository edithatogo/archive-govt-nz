...
`state_commit_status` is an explicit state machine: `not_attempted`, `committed`, `rolled_back`, `failed`, or `indeterminate`. A committed receipt requires output manifest and checkpoint roots. CAS objects may be written before canonical state commit, so CAS before/after totals remain independent from record-state commit and must always be observed.

...
- 2026-09-04: authorized run `33772828586` passed seed and parent restoration but remained stale in the 500-work acquisition step; cancellation was requested after exceeding the workflow timeout. No acquisition, reconciliation, sealing, or recovery claim is made.
- 2026-09-04: run `33772828586` reached terminal `cancelled` state after cancellation. Retained artifacts verify parent lineage/restoration and the sanitized attempt seed; reconciliation is `failed` with `mismatch_count=1`, while acquisition, verify/accounting, and continuation were skipped. The fail-closed outcome step failed as expected for a canceled acquisition. Operational proof remains blocked; do not retry without diagnosing the stale acquisition.
- 2026-09-04: PR #373 assurance run `33773918650` was observed stalled at the locked assurance gate on Windows, Ubuntu, and macOS, then reached terminal `completed / cancelled` on all three jobs after cancellation. This is not a passing hosted assurance result and does not change the operational gate.
- 2026-09-04: follow-up PR #373 assurance run `33774276982` likewise remained stalled at the locked assurance gate; GitHub finalized it `completed / cancelled` on Windows, Ubuntu, and macOS. Repeated hosted assurance cancellation remains evidence of an infrastructure/runner blockage, not a passing check or Prompt 13 operational proof.
- 2026-09-04: after the latest branch push, CI run `33774503187` and CodeQL run `33774503447` both finalized `completed / cancelled` after stalling; workflow policy lint `33774503378` passed. PR #373 therefore remains blocked by canceled required checks, not by an observed code failure.

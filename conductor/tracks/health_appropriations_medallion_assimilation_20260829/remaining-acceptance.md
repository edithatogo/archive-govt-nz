...
| AC-08 rebuildable compatibility | Verified raw-Parquet-to-SQLite/Gold/plots pipeline plus a verified canonical Budget-to-DuckDB nominal query with exact input-ID closure | Extend the canonical-recordset consumer bridge to historical/contextual measures and downstream SQLite/plots/reports; one Budget query does not complete the full bridge. |
